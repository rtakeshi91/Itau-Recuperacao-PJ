...
public class AuthInterceptor implements HandlerInterceptor {
    ...
    public boolean preHandle(HttpServletRequest request, HttpServletResponse response, Object handler) {
        ...
        if (authorizationHeader != null && authorizationHeader.startsWith("Bearer ")) {
            ...
            log.info("secret na preHandle(): {}", tokenProvider.getSecretKey());
            ...
            try {
                ...
                if (cache != null) {
                    ...
                    if (cachedLogin != null) {
                        log.info("Token recuperado do cache para usuario: {}", cachedLogin.getLogin());
                        return true;
                    }
                }
                ...
                if (tokenProvider.validarToken(token)) {
                    ...
                    log.info("Token valido para usuario: {}", usuario);

                    // Salva os dados do usuario no cache
                    if (cache != null) {
                        Login login = new Login(usuario);
                        cache.put(token, login);
                        log.info("Usuario armazenado no cache: {}", usuario);
                    }
                ...
                log.error("preHandle - Erro ao validar o token JWT: {}", e.getMessage());
            }
        } else {
            response.setStatus(HttpServletResponse.SC_UNAUTHORIZED);
            log.error("Cabeçalho 'Authorization' não encontrado ou inválido.");
        }
